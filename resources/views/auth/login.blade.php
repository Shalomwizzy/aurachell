...
<div class="min-h-screen flex">
    ...
    <div class="hidden lg:flex lg:w-[55%] relative flex-col justify-between overflow-hidden"
         style="background: linear-gradient(145deg, #2C0F0A 0%, #4a1510 50%, #1a0a06 100%);">

        {{-- Texture --}}
        <div class="absolute inset-0" style="background-image:radial-gradient(ellipse at 20% 20%, rgba(196,164,140,0.10) 0%,transparent 60%),radial-gradient(ellipse at 80% 80%, rgba(212,184,160,0.06) 0%,transparent 60%);"></div>

        {{-- Decorative rings --}}
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
            <div class="w-[700px] h-[700px] rounded-full" style="border:1px solid rgba(212,184,160,0.06);"></div>
            <div class="absolute inset-0 flex items-center justify-center">
                <div class="w-[500px] h-[500px] rounded-full" style="border:1px solid rgba(212,184,160,0.10);"></div>
            </div>
            <div class="absolute inset-0 flex items-center justify-center">
                <div class="w-[320px] h-[320px] rounded-full" style="border:1px solid rgba(212,184,160,0.16);"></div>
            </div>
        </div>
        ...
        <div class="relative z-10 p-14">
            <a href="{{ route('home') }}" class="inline-flex items-center gap-3 group">
                ...
                <span class="font-display text-xl tracking-[0.25em] uppercase transition-colors" style="color:#C4A48C;">Aurachell</span>
                @endif
            </a>
        </div>
        ...
        <div class="relative z-10 px-14 space-y-8">
            ...
            <div>
                <p class="font-sans text-[10px] tracking-[0.35em] uppercase mb-5" style="color:rgba(212,184,160,0.55);">Home Fragrance</p>
                <h2 class="font-display text-5xl leading-tight tracking-tight" style="color:#F5EDE4;">
        ...
        <div class="relative z-10 p-14 flex items-center gap-10">
            ...
            <div>
                <p class="font-display text-2xl" style="color:#C4A48C;">{{ $val }}</p>
                <p class="text-[10px] tracking-[0.2em] uppercase mt-0.5" style="color:rgba(245,237,228,0.40);">{{ $label }}</p>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="flex-1 flex flex-col" style="background:#F5EDE4;">
        ...
        <div class="flex-1 flex items-center justify-center px-8 lg:px-16 py-12">
            <div class="w-full max-w-sm">
                ...
                <div class="mb-10">
                    <h1 class="font-display text-3xl tracking-tight mb-2" style="color:#2C0F0A;">Welcome back</h1>
                    <p class="text-sm font-sans" style="color:rgba(44,15,10,0.55);">
                        New to Aurachell?
                        <a href="{{ route('register') }}" class="font-medium underline underline-offset-4 transition-colors" style="color:#6B2016;">Create account</a>
                    </p>
                </div>

                @if(session('status'))
                <div class="mb-6 px-4 py-3 border-l-2 text-sm" style="background:rgba(107,32,22,0.06);border-color:#6B2016;color:#6B2016;">{{ session('status') }}</div>
                @endif
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div>
                        <label class="block text-[10px] tracking-[0.25em] uppercase mb-3" style="color:rgba(44,15,10,0.55);">Email Address</label>
                        <input type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="email"
                               ...
                               style="border-color:rgba(107,32,22,0.20);color:#2C0F0A;"
                               onfocus="this.style.borderColor='#6B2016'"
                               onblur="this.style.borderColor='rgba(107,32,22,0.20)'">
                        @error('email')<p class="text-xs mt-2" style="color:#b91c1c;">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <label class="block text-[10px] tracking-[0.25em] uppercase" style="color:rgba(44,15,10,0.55);">Password</label>
                            @if(Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-[10px] tracking-wider underline underline-offset-4 transition-colors" style="color:rgba(107,32,22,0.65);">Forgot?</a>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="remember" id="remember"
                               class="w-4 h-4 cursor-pointer" style="accent-color:#6B2016;">
                        <label for="remember" class="text-xs cursor-pointer select-none" style="color:rgba(44,15,10,0.65);">Keep me signed in</label>
                    </div>
        ...
        <div class="px-8 lg:px-16 py-6 border-t" style="border-color:rgba(107,32,22,0.10);">
