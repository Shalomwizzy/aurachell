...
<div class="min-h-screen flex">
    ...
    <div class="hidden lg:flex lg:w-[55%] relative flex-col justify-between overflow-hidden"
         style="background:linear-gradient(145deg, #2C0F0A 0%, #371220 60%, #4A1828 100%);">

        {{-- Subtle texture rings --}}
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
            <div class="w-[700px] h-[700px] rounded-full" style="border:1px solid rgba(201,169,111,0.08);"></div>
            <div class="absolute inset-0 flex items-center justify-center">
                <div class="w-[500px] h-[500px] rounded-full" style="border:1px solid rgba(201,169,111,0.12);"></div>
            </div>
            <div class="absolute inset-0 flex items-center justify-center">
                <div class="w-[320px] h-[320px] rounded-full" style="border:1px solid rgba(201,169,111,0.18);"></div>
            </div>
        </div>
        ...
        <div class="relative z-10 px-14 space-y-8">
            ...
            <div>
                <p class="font-sans text-[10px] tracking-[0.35em] uppercase mb-5" style="color:rgba(201,169,111,0.65);">Home Fragrance</p>
        ...
        <div class="relative z-10 p-14 flex items-center gap-10">
            ...
            <div>
                ...
                <p class="text-[10px] tracking-[0.2em] uppercase mt-0.5" style="color:rgba(247,242,235,0.45);">{{ $label }}</p>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="flex-1 flex flex-col" style="background:#F7F2EB;">
        ...
        {{-- Mobile logo --}}
        <div class="lg:hidden flex items-center px-8 py-7 border-b" style="border-color:rgba(201,169,111,0.15);">
            <a href="{{ route('home') }}" class="flex items-center gap-2.5">
                @if($logo)
                <img src="{{ asset('images/' . $logo) }}" alt="Aurachell" class="h-8 w-auto">
                @else
                <div class="w-8 h-8 flex items-center justify-center rounded-sm" style="background:#371220;">
                    <span class="font-display text-sm font-bold" style="color:#F7F2EB;">A</span>
                </div>
        ...
        <div class="flex-1 flex items-center justify-center px-8 lg:px-16 py-12">
            <div class="w-full max-w-sm">
                ...
                <div class="mb-10">
                    <h1 class="font-display text-3xl tracking-tight mb-2" style="color:#371220;">Welcome back</h1>
                    <p class="text-sm font-sans" style="color:rgba(55,18,32,0.55);">
                        New to Aurachell?
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div>
                        <label class="block text-[10px] tracking-[0.25em] uppercase mb-3" style="color:rgba(55,18,32,0.55);">Email Address</label>
                        <input type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="email"
                               ...
                               style="border-color:rgba(55,18,32,0.20);color:#371220;"
                               onfocus="this.style.borderColor='#371220'"
                               onblur="this.style.borderColor='rgba(55,18,32,0.20)'">
                        @error('email')<p class="text-xs mt-2" style="color:#6B2016;">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <div class="flex items-center justify-between mb-3">
                            <label class="block text-[10px] tracking-[0.25em] uppercase" style="color:rgba(55,18,32,0.55);">Password</label>
                            @if(Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-[10px] tracking-wider underline underline-offset-4 transition-colors" style="color:rgba(55,18,32,0.55);">Forgot?</a>
                            @endif
                        </div>
                        <input type="password" name="password" required autocomplete="current-password"
                               ...
                               onfocus="this.style.borderColor='#371220'"
                               onblur="this.style.borderColor='rgba(55,18,32,0.20)'">
                        @error('password')<p class="text-xs mt-2" style="color:#6B2016;">{{ $message }}</p>@enderror
                    </div>

                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="remember" id="remember"
                               class="w-4 h-4 cursor-pointer" style="accent-color:#371220;">
                        <label for="remember" class="text-xs cursor-pointer select-none" style="color:rgba(55,18,32,0.65);">Keep me signed in</label>
                    </div>
        ...
        <div class="px-8 lg:px-16 py-6 border-t" style="border-color:rgba(55,18,32,0.10);">
            <p class="text-center text-xs" style="color:rgba(55,18,32,0.40);">© {{ date('Y') }} Aurachell. All rights reserved.</p>
        </div>
    </div>
</div>
