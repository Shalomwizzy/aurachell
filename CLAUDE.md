...
- [x] Bank-transfer proof notification to admin fixed — (1) was `->queue()` so it never sent unless the queue worker/scheduler cron was running on the server (root cause: `QUEUE_CONNECTION=database` + likely no `schedule:run` cron); now `->send()` synchronously. (2) The uploaded **screenshot is now attached** to the email (`BankTransferSubmittedMail::attachments()` reads `storage/app/bank-transfer-proofs/{proof_path}`). (3) Template rewritten fully inline (head-CSS-stripping webmail) with customer name/email/amount/reference/note. NOTE: all **transactional/action-triggered** emails now send synchronously (`->send()`) so they never depend on the cron — order confirmation, admin order notification, bank-transfer proof/approved/rejected, returns submit/status, order status updates, welcome. Only **time-based campaign** emails remain `->queue()` (Console/Commands: birthday, festive, cart-abandonment, review-request, wishlist, back-in-stock, new-product, new-month, reorder) — these genuinely need the scheduler cron `* * * * * schedule:run` (custom cron on Hostinger, `/opt/alt/php84/usr/bin/php`, since `/usr/bin/php` is 8.3 and app needs 8.4).
- [x] Shared **inline** email layout `emails/layouts/inline.blade.php` (header/footer/hero via inline styles, no `<head>` CSS) — all status/delivery/admin emails now extend it so they survive style-stripping webmail. Rebuilt on it: `order-status-update`, `order-shipped`, `delivery-completed`, `admin-delivery-notification`, `admin-review-notification`, `admin-preorder-notification`. (The premium base layout `layouts/base.blade.php` still exists but relies on head CSS — prefer `layouts/inline` for anything the client actually receives.)
- [x] Customer **phone** now surfaced everywhere it was missing — `Order::getCustomerPhoneAttribute()` (user phone → guest_phone → shipping_address['phone']); shown in admin order-detail customer card + shipping block, and in `admin-order-notification`, `admin-delivery-notification`, `admin-preorder-notification` emails
- [x] Product image **delete button fixed** — the `Set Primary` hover overlay was `absolute inset-0` (covering the whole thumbnail incl. the × button) and intercepted its click, so only primary images (which have no overlay) were deletable. Overlay is now `pointer-events-none` (button inside `pointer-events-auto`) and the × button is `z-20` — every image is deletable. Categories use a single replaceable image, no such grid.
- [x] `new-month` + `birthday` emails rebuilt fully **inline** — both extended the base layout (head-CSS-stripped in webmail) and new-month also used `display:flex` (unsupported in email). Now self-contained inline table layouts; birthday keeps its maroon hero + optional coupon block.
- [x] Admin order-notification email now fully **inline-styled** — the client's webmail (Hostinger/Titan) strips `<head><style>`, so the base-layout classes (logo, colours, layout) all collapsed. `admin-order-notification.blade.php` rewritten as self-contained table + inline styles (no head-CSS dependency, gold text-logo fallback visible). NOTE: other admin-received emails (`admin-delivery`, `admin-review`, `admin-preorder`, `bank-transfer-submitted`) still use the base layout and would show the same breakage in a style-stripping client — inline them if the client reports issues.
- [x] Admin notifications now go to **hello@ only** — removed the `User::role('sales_rep')->get()` loop from `PaymentLifecycleService`, `ReconcilePaystackOrders`, and `PreorderController` (deactivating a sales_rep did NOT stop mail — the query ignored `is_blocked`, and the old sales@ user still exists in prod DB). All admin mail now targets `config('services.admin.email')` only.
- [x] Product Performance page — `Admin/ProductPerformanceController` + `admin/product-performance/index.blade.php`, route `admin.product-performance.index` (Finance group, gated `reports.view`). Lists **every** product (LEFT JOIN so zero-sales products show) with units sold, orders, revenue, stock over a period filter (7/30/90/all); sortable by best/least selling, most units, lowest stock, name; KPI tiles (total revenue, units, no-sales count, best seller); CSV export (`admin.product-performance.export`, respects current period + sort); complements the top-5 dashboard widget + top-10 Reports section which only show best sellers
