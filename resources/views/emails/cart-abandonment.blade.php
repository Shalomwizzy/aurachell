...
<div style="display:flex;justify-content:space-between;align-items:center;padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.05);">
    <div>
        <p style="margin:0 0 2px;font-size:14px;color:#2C0F0A;">{{ $item->product?->name ?? 'Product' }}</p>
        <p style="margin:0;font-size:12px;color:rgba(44,15,10,0.60);font-family:Arial,sans-serif;">Qty: {{ $item->quantity }}</p>
    </div>
    <p style="margin:0;font-size:14px;color:#C4A48C;">₦{{ number_format(($item->price ?? 0) * $item->quantity) }}</p>
</div>
@endforeach
...
<p style="margin-top:24px;font-size:13px;text-align:center;color:rgba(44,15,10,0.55);">
