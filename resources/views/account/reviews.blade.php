...
<div class="bg-white shadow-luxury p-5 mb-4">
    <div class="flex items-start justify-between gap-4 flex-wrap mb-3">
        ...
        <span class="text-[10px] tracking-widest uppercase px-2.5 py-1 font-sans {{ $review->is_approved ? 'bg-green-100 text-green-700' : 'bg-sand/40 text-text-muted' }}">
    ...
    <div class="flex items-center gap-0.5 mb-2">
        @for($i = 1; $i <= 5; $i++)
        <svg class="w-4 h-4 {{ $i <= $review->rating ? 'text-yellow-400' : 'text-sand/50' }}" fill="currentColor" viewBox="0 0 20 20">
