...
<p>Wonderful news, <strong style="color:#1E0C14;">{{ $order->customer_name }}</strong>. Your Aurachell order has been carefully packed and handed over for delivery.</p>

{{-- Tracking box --}}
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"
       style="background:rgba(212,185,154,0.12);border:1px solid rgba(201,169,111,0.22);padding:28px;margin:24px 0;text-align:center;">
    <tr>
        <td>
            @if($order->tracking_code)
            <span class="label">Your Tracking Code</span>
            <p style="font-family:Arial,sans-serif;font-size:22px;font-weight:700;color:#C9A96F;letter-spacing:0.12em;margin:10px 0 6px;">
                {{ $order->tracking_code }}
            </p>
            @else
            <span class="label">Order Reference</span>
            <p style="font-family:Arial,sans-serif;font-size:20px;font-weight:700;color:#C9A96F;letter-spacing:0.08em;margin:10px 0 6px;">
                {{ $order->order_number }}
            </p>
            @endif
            <p style="font-family:Arial,sans-serif;font-size:11px;color:rgba(30,12,20,0.45);margin:0;letter-spacing:0.10em;">
...
<table class="order-table">
    <tbody>
        ...
        <tr>
            ...
            <td style="text-align:right;color:rgba(30,12,20,0.55);">× {{ $item->quantity }}</td>
        </tr>
        @endforeach
    </tbody>
</table>
...
{{-- Tips while waiting --}}
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"
       style="background:rgba(55,18,32,0.04);border-left:2px solid #C9A96F;padding:20px 24px;margin:0 0 24px;">
    <tr>
        <td>
