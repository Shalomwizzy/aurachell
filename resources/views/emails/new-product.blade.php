...
<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"
    ...
    <tr>
        <td style="padding:28px 32px;">
            @if($product->category)
            <p style="font-family:Arial,sans-serif;font-size:9px;letter-spacing:0.28em;text-transform:uppercase;color:rgba(201,169,111,0.75);margin:0 0 10px;">
                {{ $product->category->name }}
            </p>
            @endif
            <h2 style="font-family:Georgia,serif;font-size:22px;color:#1E0C14;font-weight:normal;margin:0 0 12px;line-height:1.3;">
                {{ $product->name }}
            </h2>
            @if($product->short_description)
            <p style="font-family:Arial,sans-serif;font-size:13px;color:rgba(30,12,20,0.58);margin:0 0 20px;line-height:1.7;">
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                    <td style="vertical-align:middle;">
                        <p style="font-family:Georgia,serif;font-size:22px;color:#C9A96F;margin:0;font-weight:600;">
                            ₦{{ number_format($product->price) }}
                        </p>
                        @if(isset($product->compare_at_price) && $product->compare_at_price > $product->price)
                        <p style="font-family:Arial,sans-serif;font-size:12px;color:rgba(30,12,20,0.40);text-decoration:line-through;margin:3px 0 0;">
...
<p style="font-family:Georgia,serif;font-size:14px;color:rgba(30,12,20,0.65);line-height:1.7;margin:0 0 24px;">
    {{ $product->scent_notes }}
</p>
@endif
