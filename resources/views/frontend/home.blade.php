...
<section class="relative min-h-[92vh] flex items-center justify-center overflow-hidden"
    ...
    <div class="absolute inset-0" style="background:radial-gradient(ellipse at 30% 50%,rgba(212,185,154,0.12) 0%,transparent 60%),radial-gradient(ellipse at 70% 20%,rgba(201,169,111,0.08) 0%,transparent 50%)"></div>

    <div class="relative z-10 max-w-4xl mx-auto px-6 text-center animate-fade-in">
        <p class="font-sans text-xs tracking-[0.35em] uppercase mb-6" style="color:rgba(212,185,154,0.90)">Luxury Home Fragrance · Nigeria</p>
        <h1 class="font-display text-5xl sm:text-6xl lg:text-7xl leading-[1.1] tracking-tight text-balance mb-6" style="color:#FAF5ED">
            Crafted for Calm.<br>
            <em style="color:#C9A96F">Designed for Home.</em>
        </h1>
        <p class="font-sans text-lg max-w-lg mx-auto leading-relaxed mb-10" style="color:rgba(250,245,237,0.82)">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="{{ route('shop') }}" class="btn-primary px-10 py-4" style="background:#C9A96F;color:#1E0C14;">
                Shop the Collection
            </a>
            <a href="{{ route('about') }}" class="inline-flex items-center gap-2 font-sans text-sm tracking-widest uppercase transition-colors" style="color:rgba(250,245,237,0.60)">
                Our Story
    ...
    <div class="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-bounce" style="color:rgba(250,245,237,0.3)">
...
<div class="py-4 overflow-hidden" style="background:var(--color-primary)">
    <div class="flex items-center gap-12 text-xs tracking-[0.3em] uppercase font-sans text-center" style="color:rgba(250,245,237,0.85)">
        ...
        <span class="w-1 h-1 rounded-full flex-shrink-0" style="background:rgba(250,245,237,0.40)"></span>
        @endforeach
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <a href="{{ route('shop', ['category' => $cat->slug]) }}" class="group relative overflow-hidden aspect-[4/5] block"
           style="background:var(--color-base)">
            ...
            @endif
            <div class="absolute inset-0 transition-opacity duration-300" style="background:linear-gradient(to top,rgba(30,12,20,0.82) 0%,rgba(30,12,20,0.2) 60%,transparent 100%)"></div>
            <div class="absolute bottom-0 left-0 right-0 p-6">
                <h3 class="font-display text-2xl mb-1" style="color:#FAF5ED">{{ $cat->name }}</h3>
                <p class="text-sm" style="color:rgba(250,245,237,0.65)">{{ $cat->description }}</p>
                <span class="inline-flex items-center gap-1 text-xs tracking-widest uppercase mt-3 font-medium group-hover:gap-2 transition-all" style="color:#C9A96F">
                    Shop Now
...
{{-- ===== BESTSELLERS ===== --}}
<section class="py-20" style="background:var(--color-base);background:linear-gradient(180deg,var(--color-surface) 0%,rgba(212,185,154,0.15) 100%)">
            ...
            <div class="card-product" x-data="{ wishlist: false }">
                <a href="{{ route('product.show', $product->slug) }}" class="block">
                    <div class="relative overflow-hidden aspect-square" style="background:var(--color-base)">
                        <img src="{{ $product->primary_image_url }}" alt="{{ $product->name }}"
                            ...
                            loading="lazy"
                            onerror="this.src='https://placehold.co/400x400/D4B99A/371220?text=Aurachell'">
                ...
                <div class="px-4 pb-4"
                    ...
                    <button
                        ...
                        :style="added
                            ? 'background:var(--color-primary);color:#FAF5ED;border:1px solid var(--color-primary);'
...
<section class="py-20" style="background:var(--color-primary)">
    <div class="max-w-4xl mx-auto px-6 text-center">
        <p class="font-sans text-xs tracking-[0.3em] uppercase mb-6" style="color:rgba(212,185,154,0.90)">Our Philosophy</p>
        <h2 class="font-display text-3xl md:text-5xl leading-tight mb-8" style="color:#FAF5ED">
            "Every scent is a story.<br>
            <em style="color:#C9A96F">Every space, a sanctuary."</em>
        </h2>
        <p class="text-base leading-relaxed max-w-2xl mx-auto mb-10" style="color:rgba(250,245,237,0.82)">
        ...
        <a href="{{ route('about') }}" class="btn-primary" style="background:#C9A96F;color:#1E0C14;">
            Read Our Story
        </a>
    </div>
</section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="p-6 rounded-sm" style="background:white;box-shadow:var(--shadow-luxury)">
                <div class="flex items-center gap-1 mb-4">
                    @for($i = 0; $i < 5; $i++)
                    <svg class="w-4 h-4" style="color:{{ $i < $review->rating ? '#371220' : 'rgba(55,18,32,0.20)' }}" fill="currentColor" viewBox="0 0 20 20">
