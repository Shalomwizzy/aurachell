...
<head>
    ...
    <style>
        :root {
            ...
            --adm-muted:        rgba(247,242,235,0.50);
            --adm-accent:       #371220;
            --adm-gold:         #C9A96F;
            --adm-success-bg:   rgba(55,18,32,0.12);
            --adm-success-fg:   rgba(247,242,235,0.85);
            --adm-warn-bg:      rgba(55,18,32,0.12);
            --adm-warn-fg:      rgba(247,242,235,0.85);
            --adm-danger-bg:    rgba(201,169,111,0.10);
            --adm-danger-fg:    rgba(247,242,235,0.85);
            --adm-info-bg:      rgba(55,18,32,0.10);
            --adm-info-fg:      rgba(247,242,235,0.85);
        }
        ...
        body .text-text-dark/60,
        body .text-text-muted,
        body .text-text-muted,
        body .text-warmSand-300,
        body .text-white\/60,
        body .text-white\/55,
        body .text-white\/50,
        body .text-white\/45,
        ...
        body .hover\:bg-sage-800:hover { background-color: var(--adm-accent) !important; color: #F7F2EB !important; }
        body .text-cream { color: #F7F2EB !important; }
        ...
        <nav class="flex-1 py-3 space-y-0.5 overflow-y-auto">
            ...
            <div class="px-2 space-y-0.5">
                ...
                <a href="{{ route('admin.products.low-stock') }}" class="{{ $cr === 'admin.products.low-stock' ? 'adm-nav-item active' : 'adm-nav-item' }}">
                    ...
                    @if($lowStockNav > 0)<span class="ml-auto text-[10px] px-1.5 py-0.5 rounded-full" style="background:rgba(55,18,32,0.35);color:var(--adm-gold);">{{ $lowStockNav }}</span>@endif
                </a>
            ...
            <div class="px-2 space-y-0.5">
                ...
                <a href="{{ route('admin.orders.index', ['payment_status'=>'paid']) }}" class="{{ ($cr==='admin.orders.index'&&request('payment_status')==='paid') ? 'adm-nav-item active' : 'adm-nav-item' }}">
                    ...
                    @if($paidOrders > 0)<span class="ml-auto text-[10px] px-1.5 py-0.5 rounded-full" style="background:rgba(55,18,32,0.35);color:var(--adm-gold);">{{ $paidOrders }}</span>@endif
                </a>
            ...
            <div class="px-2 space-y-0.5">
                ...
                {!! adminNavItem('admin.product-performance.index', 'Product Performance', 'M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z', $cr) !!}
        ...
        <nav id="adm-nav" class="flex-1 py-3 space-y-0.5 overflow-y-auto"
                ...
                <div id="adm-gcontent-catalog" class="px-2 space-y-0.5">
                    ...
                    <a href="{{ route('admin.products.low-stock') }}"
                       class="{{ $currentRoute === 'admin.products.low-stock' ? 'adm-nav-item active' : 'adm-nav-item' }}">
                        ...
                        <span class="ml-auto text-[10px] px-1.5 py-0.5 rounded-full" style="background:rgba(55,18,32,0.35);color:var(--adm-gold);">{{ $lowStockNav }}</span>
                        @endif
                    </a>
                ...
                <div id="adm-gcontent-orders" class="px-2 space-y-0.5">
                    ...
                    <a href="{{ route('admin.orders.index', ['payment_status'=>'paid']) }}"
                       class="{{ ($currentRoute === 'admin.orders.index' && request('payment_status')==='paid') ? 'adm-nav-item active' : 'adm-nav-item' }}">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 8h6m-5 0a3 3 0 110 6H9l3 3m-3-6h6m6 1a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        <span class="nav-label">Paid</span>
                        @if($paidOrders > 0)<span class="ml-auto text-[10px] px-1.5 py-0.5 rounded-full nav-label" style="background:rgba(55,18,32,0.35);color:var(--adm-gold);">{{ $paidOrders }}</span>@endif
                    </a>
            ...
            <div class="mt-1">
                ...
                <div id="adm-gcontent-finance" class="px-2 space-y-0.5">
                    ...
                    {!! adminNavItem('admin.product-performance.index', 'Product Performance', 'M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z', $currentRoute) !!}
                    @endif
                    @if($u->hasAnyRole(['super_admin','admin']))
            ...
            <div class="mt-1">
                ...
                <div id="adm-gcontent-customers" class="px-2 space-y-0.5">
                    ...
                    {!! adminNavItem('admin.wishlists.index', 'Wishlists', 'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z', $currentRoute) !!}
                    @endif
                ...
                <a href="{{ route('admin.messages.index') }}" class="relative"
                   ...
                   onmouseover="this.style.color='var(--adm-gold)'" onmouseout="this.style.color='var(--adm-muted)'">
                    ...
                    <span class="absolute -top-1 -right-1 w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold pulse"
                          style="background:#C9A96F;color:#160c0b;">{{ $unread }}</span>
                    @endif
                </a>
