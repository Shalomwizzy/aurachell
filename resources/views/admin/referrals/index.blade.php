...
<div class="p-6 space-y-6 max-w-7xl">
    ...
    <div class="flex items-center justify-between gap-4 flex-wrap">
        ...
        <a href="{{ route('admin.referrals.settings') }}"
           ...
           style="background:rgba(196,164,140,0.10);color:var(--adm-gold);border:1px solid rgba(196,164,140,0.25);"
           onmouseover="this.style.background='rgba(196,164,140,0.18)'" onmouseout="this.style.background='rgba(196,164,140,0.10)'">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        @foreach([
            ['label' => 'Total Sign-ups',   'value' => $stats['total'],    'sub' => 'via referral code',      'color' => 'rgba(196,164,140,0.15)', 'icon' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z'],
            ...
            ['label' => 'Conversion Rate',  'value' => $convRate . '%',    'sub' => 'sign-ups → purchases',   'color' => 'rgba(107,32,22,0.12)',    'icon' => 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6'],
        ] as $card)
        <div class="rounded-sm p-4" style="background:var(--adm-surface-alt);border:1px solid var(--adm-border);">
            ...
            <div class="mt-3 h-1 rounded-full overflow-hidden" style="background:rgba(196,164,140,0.15);">
    ...
    <div class="rounded-sm px-5 py-4 flex flex-wrap items-center gap-6" style="background:rgba(107,32,22,0.08);border:1px solid rgba(107,32,22,0.18);">
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 rounded-sm overflow-hidden" style="background:var(--adm-surface-alt);border:1px solid var(--adm-border);">
            <div class="px-5 py-4 border-b flex items-center justify-between" style="border-color:var(--adm-border);">
                ...
                <span class="text-[11px] px-2.5 py-1 rounded-full" style="background:rgba(196,164,140,0.12);color:var(--adm-muted);">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom:1px solid var(--adm-border);" class="transition-colors hover:bg-[rgba(212,184,160,0.03)]">
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-2.5">
                                    <div class="w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-semibold flex-shrink-0"
                                         style="background:rgba(196,164,140,0.15);color:var(--adm-muted);">
                            ...
                            <td class="px-4 py-3.5">
                                ...
                                <code class="text-[11px] px-2 py-1 rounded-sm font-mono tracking-wider" style="background:rgba(107,32,22,0.10);color:var(--adm-gold);">{{ $referral->reward_coupon_code }}</code>
                                @else
                ...
                <div class="px-5 py-3.5 {{ !$loop->last ? 'border-b' : '' }}" style="border-color:var(--adm-border);">
                    <div class="flex items-center gap-3 mb-2">
                        <span class="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0"
                              style="{{ $i === 0 ? 'background:rgba(196,164,140,0.3);color:var(--adm-gold);' : 'background:rgba(196,164,140,0.1);color:var(--adm-muted);' }}">
                            {{ $i + 1 }}
                        </span>
                    ...
                    <div class="h-1 rounded-full overflow-hidden" style="background:rgba(196,164,140,0.12);">
                        <div class="h-full rounded-full" style="width:{{ round($user->rewarded_count / $maxCount * 100) }}%;background:{{ $i === 0 ? 'var(--adm-gold)' : 'rgba(196,164,140,0.4)' }};transition:width .4s;"></div>
                    </div>
                </div>
                @empty
                ...
                <div class="px-5 py-4 space-y-4">
                    ...
                    <div class="flex gap-3">
                        <div class="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold flex-shrink-0 mt-0.5"
                             style="background:rgba(107,32,22,0.15);color:var(--adm-gold);">{{ $step }}</div>
                        <div>
