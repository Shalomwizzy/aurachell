...
<div class="p-6 lg:p-8 max-w-2xl">
    ...
    <div class="flex items-center gap-3 mb-8">
        <a href="{{ route('admin.coupons.index') }}" class="text-white/30 hover:text-white/60 transition-colors">
    ...
    <form method="POST" action="{{ route('admin.coupons.update', $coupon) }}" class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $coupon->code) }}" required
                       ...
                       class="w-full bg-white/5 border border-white/10 px-4 py-2.5 text-sm text-white/85 font-mono tracking-widest focus:outline-none focus:border-[#D9C5A0]/40 @error('code') border-red-500/40 @enderror">
                @error('code')<p class="text-red-400/80 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-[10px] tracking-[0.2em] uppercase text-white/40 mb-2">Type *</label>
                <select name="type" required
                        class="w-full bg-white/5 border border-white/10 px-4 py-2.5 text-sm text-white/85 focus:outline-none focus:border-[#D9C5A0]/40">
        ...
        <div class="pt-4 flex gap-3">
            ...
            <a href="{{ route('admin.coupons.index') }}"
               class="px-8 py-3 text-xs tracking-[0.2em] uppercase text-white/40 border border-white/10 hover:border-white/20 hover:text-white/60 transition-colors">
                Cancel
            </a>
        </div>
    </form>
