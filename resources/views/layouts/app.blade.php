...
<body class="bg-surface">
    ...
    <header id="main-header" class="sticky top-0 z-50 transition-all duration-300" style="background:var(--color-bg);">
        <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16 lg:h-20">
                ...
                <a href="{{ route('home') }}" class="flex-shrink-0 min-w-0 flex flex-col items-start justify-center leading-none py-1">
                    @if($logo)
                    <img src="{{ asset('images/' . $logo) }}" alt="Aurachell" class="h-8 sm:h-10 w-auto object-contain dark:brightness-0 dark:invert block" style="max-width:140px;">
                    @else
                    <span class="font-display text-lg sm:text-2xl text-sage tracking-wider">Aurachell</span>
                    @endif
                    <span class="font-sans text-[9px] sm:text-[10px] tracking-[0.18em] uppercase mt-1.5 whitespace-nowrap font-medium"
                          style="color:var(--color-primary);">For Moments That Stay With You</span>
                </a>
    ...
    <a href="https://wa.me/{{ preg_replace('/\D/', '', $waNumber) }}?text=Hi%20Aurachell%2C%20I%20have%20a%20question"
       ...
       style="position:fixed;bottom:1.5rem;left:1.5rem;z-index:50;width:3.5rem;height:3.5rem;border-radius:9999px;display:flex;align-items:center;justify-content:center;background:#25D366;box-shadow:0 8px 60px rgba(42,37,34,.14);transition:transform .2s;"
    ...
    {{-- AI Chatbot --}}
    <div style="position:fixed;bottom:1.5rem;right:1.5rem;z-index:60;">
